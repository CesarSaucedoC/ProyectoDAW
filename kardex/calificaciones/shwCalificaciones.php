...
session_start();
  ...
  $qry = 'SELECT calificacion.matricula,CONCAT_WS(\' \',alumno.nombre, alumno.paterno,alumno.materno) AS \'alumno\', curso.nombre, CONCAT_WS(\' \',profesor.nombre, profesor.materno, profesor.paterno) AS \'profesor\', calificacion.periodo, calificacion.calif FROM calificacion,alumno,profesor,curso WHERE calificacion.matricula=alumno.matricula AND calificacion.profesor=profesor.clave AND calificacion.curso=curso.clave ORDER BY calificacion.matricula;';
